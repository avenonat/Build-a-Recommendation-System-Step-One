...
public class FirstRatings
{
    ...
    public void testLoadRaters() {
        ...
        for (Rater red : list) {
            System.out.println(red.getID() + " " + red.getMyRatings());
        }
        System.out.println("In the file: " + list.size() + " raters.");
        for (Rater temp : list) {
            if (temp.getID().equals("2")) {
                System.out.println("Was rated: " + temp.getMyRatings().size());
            }
            if (temp.getMyRatings().size() > max) {
                max = temp.getMyRatings().size();
                max_id = temp.getID();
            }
            ArrayList<Rating> test = temp.getMyRatings();
            for (Rating s : test) {
                if (s.getItem().equals("1798709")) {
                    count += 1;
                }
                String film = s.getItem();
    ...
    public ArrayList<Rater> csvMethod2(CSVParser parser){
        ...
        for (Map.Entry<String, ArrayList<Rating>> set : step.entrySet()) {
            Rater fin = new Rater(set.getKey());
